 ...
 * ฟังก์ชันจำลองตรรกะการตรวจสอบข้อมูลหน้า Login
 * - ตรวจสอบว่ากรอก Username และ Password หรือไม่
 * - ถ้าขาดอย่างใดอย่างหนึ่ง หรือมีเพียงช่องว่าง จะส่งคืนสถานะ ok: false พร้อมข้อความแจ้งเตือน
 ...
 * ฟังก์ชันจำลองตรรกะการตรวจสอบข้อมูลหน้า Register
 * - ตรวจสอบความยาวของ Username (ต้อง >= 4 ตัวอักษร)
 * - ตรวจสอบความซับซ้อนของ Password (ต้องมีตัวพิมพ์เล็ก, ตัวพิมพ์ใหญ่, ตัวเลข, และยาว 8 ตัวขึ้นไป)
 * - ตรวจสอบว่า Password และ Confirm Password ตรงกันหรือไม่
 * - หากไม่ผ่านเงื่อนไข จะคืนค่าสถานะ ok: false พร้อมระบุฟิลด์ที่ผิดพลาดและข้อความแจ้งเตือน
 ...
 * ฟังก์ชันจำลองตรรกะการตรวจสอบรหัสเข้าร่วมการทำแบบทดสอบ (Join Quiz)
 * - ตรวจสอบว่ามีการกรอกรหัสข้อสอบหรือไม่ (ห้ามเป็นค่าว่าง)
 * - ค้นหารหัสในฐานข้อมูล (จำลองโดยใช้ storage) ว่ามีข้อสอบตรงกับรหัสนั้นหรือไม่
 * - ตรวจสอบว่าข้อสอบนั้นมีการตั้งคำถามไว้หรือไม่
 * - คืนค่า ok: true พร้อมข้อมูลข้อสอบที่ค้นพบ หากรหัสถูกต้องและพร้อมใช้งาน
 ...
 * ฟังก์ชันจำลองตรรกะการจัดการสถานะขณะทำแบบทดสอบ (Play Quiz)
 * - สร้าง object เพื่อติดตามคำตอบในแต่ละข้อ (รองรับการเปลี่ยนใจแก้ไขคำตอบ)
 * - นับจำนวนข้อที่ตอบไปแล้ว และคำนวณเปอร์เซ็นต์ความคืบหน้า (Progress)
 * - จัดการการเปลี่ยนไปยังคำถามข้อถัดไป และสามารถตรวจสอบได้ว่าตัวเลือกไหนถูกเลือกอยู่
 */
...
//  อธิบายเพิ่มเติม: ทดสอบระบบตรวจสอบข้อมูลหน้า Login เพื่อป้องกันไม่ให้ผู้ใช้งาน
//  ส่งฟอร์มเข้าสู่ระบบหากไม่ได้กรอก Username หรือ Password (หรือกรอกแค่ช่องเดียว)
// ═════════════════════════════════════════════════════════════════════════════
...
//  อธิบายเพิ่มเติม: ทดสอบระบบตรวจสอบข้อมูลหน้าสมัครสมาชิก (Register) เพื่อให้แน่ใจว่า
//  ผู้ใช้ตั้งรหัสผ่านตรงตามเงื่อนไข (มีความปลอดภัย) และกรอกรหัสผ่านยืนยันได้ตรงกัน
// ═════════════════════════════════════════════════════════════════════════════
...
//  อธิบายเพิ่มเติม: ทดสอบระบบเข้าร่วมห้องสอบ (Join) ว่าสามารถดักจับการกรอกรหัสข้อสอบ
//  ที่เป็นค่าว่าง (Empty), ค่า Null, หรือมีแต่ช่องว่าง (Whitespace) ได้ถูกต้อง
// ═════════════════════════════════════════════════════════════════════════════
...
//  อธิบายเพิ่มเติม: ทดสอบระบบตรวจสอบรหัสข้อสอบ (Join Code) ในกรณีต่างๆ เช่น
//  กรอกรหัสผิด(ไม่มีในระบบ), กรอกรหัสถูก(เข้าห้องสอบได้), และรหัสถูกแต่ข้อสอบยังไม่มีคำถาม
// ═════════════════════════════════════════════════════════════════════════════
...
//  อธิบายเพิ่มเติม: ทดสอบตรรกะเบื้องหลังหน้าทำข้อสอบ (Play Quiz) ว่าเมื่อผู้ใช้กดเลือก
//  คำตอบแล้ว ระบบบันทึก State ได้ถูกต้อง เปลี่ยนใจเลือกข้ออื่นได้ และคำนวณ Progress Bar ถูกต้อง
// ═════════════════════════════════════════════════════════════════════════════
